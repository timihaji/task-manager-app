...
function LeftNav({ tasks, view, onView, collapsed, onSettings, activeLifeAreas, onLifeAreaToggle, theme }) {
  ...
  const counts = {
    ...
    snoozed: all.filter(t=>!!t.snoozedUntil&&!t.done&&!t.parentId&&!t.delegatedTo).length,
    someday: all.filter(t=>!!t.someday&&!t.done&&!t.parentId&&!t.delegatedTo).length,
    blocked: all.filter(t=>t.blocked&&!t.done&&!t.parentId&&!t.delegatedTo).length,
